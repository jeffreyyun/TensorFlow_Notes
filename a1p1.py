...
m = tf.random_normal([10, 10], mean=0, stddev=1)
...
unique_values, indices = tf.unique(x)	# returns a tuple
...
neg_av = tf.reduce_mean(tf.square(x - y))
other_av = tf.reduce_sum(tf.abs(x - y))
out = tf.where(cond, neg_av, other_av) # replaces tf.select() in r1.x
